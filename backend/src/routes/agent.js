...
function generateUniversalScript(serverUrl, regKey) {
  return [
    ...
    '            $csessions = $raw -replace "^[> ]", "" | ForEach-Object {',
    '              if ($_ -match "^[^A-Za-z0-9]*(\S+)\s+(\d+)\s+\S+") { [int]$matches[2] }',
    '            } | Where-Object { $_ -ne $null -and $_ -gt 0 -and $_ -lt 65500 }',
    '            Write-Log "To logoff: $([string]::Join(\",\", $csessions))"',
    '            foreach ($id in $csessions) {',
    '              try { & C:\\Windows\\System32\\logoff.exe $id 2>&1 | Out-Null; Write-Log "Logoff session $id OK" }',
    '              catch { Write-Log "Logoff session $id FAIL: $_" }',
    '            }',
    '          }',
    '        } else {',
