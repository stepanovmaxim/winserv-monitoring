...
function generateUniversalScript(serverUrl, regKey) {
  return [
    ...
    '            $csessions = $raw | Select-String "\s+(\d+)\s+(Active|Conn|Disc)" | ForEach-Object { [int]$_.Matches.Groups[1].Value } | Where-Object { $_ -gt 0 }',
    ...
    '              catch { Write-Log "Logoff session $id FAIL" }',
    '            }',
    '            Write-Log "Logoff completed"',
    '          }',
    '        } else {',
