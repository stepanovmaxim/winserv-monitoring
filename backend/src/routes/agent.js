...
function generateUniversalScript(serverUrl, regKey, fallbackUrl) {
  return [
    ...
    '  # Restrict the agent directory, the script, the stored token and the log to',
    '  # SYSTEM only, so an ordinary user - or an ordinary administrator who simply',
    '  # opens the file - is denied. Runs as SYSTEM (the scheduled task), which',
    '  # keeps FullControl, so self-update still rewrites the script normally.',
    '  #',
    '  # This is the real barrier for ordinary accounts. It is NOT absolute: an',
    '  # administrator can take ownership and grant themselves back in. That is',
    '  # inherent to the Windows model and cannot be prevented from inside a',
    '  # process the same admin controls; secrets are machine-bound above so what',
    '  # they could recover does not work anywhere else.',
    '  #',
    '  # By SID S-1-5-18, never the name "SYSTEM": the account is localised.',
    '  try {',
    '    $targets = @()',
