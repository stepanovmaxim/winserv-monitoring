...
function generateUniversalScript(serverUrl, regKey, fallbackUrl) {
  return [
    ...
    '  # Restrict the agent directory, script, token file and log to SYSTEM only,',
    '  # so an ordinary user - or an admin who merely opens the file - is denied.',
    '  # Runs as SYSTEM (the task), which keeps FullControl, so self-update still',
    '  # rewrites the script. By SID S-1-5-18: the account name is localised.',
    '  #',
    '  # Applied once, then recorded by a marker file inside the (now SYSTEM-only)',
    '  # directory. Reading the ACL back to decide proved unreliable across',
    '  # locales; a marker is language independent and a non-admin cannot forge it',
    '  # inside a SYSTEM-only folder.',
    '  #',
    '  # Not absolute: an administrator can take ownership and grant themselves',
    '  # back. That is inherent to Windows; secrets are machine-bound instead.',
    '  try {',
    '    $ad = Split-Path $PSCommandPath -Parent',
    '    if (-not $ad) { $ad = "C:\\winserv-agent" }',
    '    $cd = Split-Path $ConfigFile -Parent',
    '    $marker = Join-Path $ad ".acllock"',
