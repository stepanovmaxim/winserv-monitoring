...
function generateUniversalScript(serverUrl, regKey) {
  return [
    ...
    '            $raw = @(quser 2>$null)',
    '            Write-Log "quser count: $($raw.Count)"',
    '            if ($raw.Count -gt 0) {',
    '              $testLine = $raw[1].ToString()',
    '              $testMatch = $testLine -match "\s+(\d+)\s+\S"',
    '              Write-Log "test: $testMatch, matches: $($matches[1])"',
    '            }',
    '            $csessions = foreach ($item in $raw) {',
    '              $s = $item.ToString()',
    '              if ($s -match "\s+(\d+)\s+\S") { $id = [int]$matches[1]; if ($id -gt 0 -and $id -lt 65500) { $id } }',
    '            }',
