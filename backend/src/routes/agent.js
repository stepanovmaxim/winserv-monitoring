...
function generateUniversalScript(serverUrl, regKey) {
  return [
    ...
    '              Write-Log "TS: WMI available"',
    '            } catch {}',
    '            # Try quser first, fallback to qwinsta',
    '            $allLines = @(quser 2>$null)',
    '            if ($allLines.Count -le 1) { $allLines = @(query session 2>$null) }',
    '            Write-Log "Got $($allLines.Count) session lines"',
    '            # Extract anything that looks like a session ID',
    '            $allLines | Select-String "\b(\d{1,5})\b" | ForEach-Object {',
    '              $val = $_.Matches.Value',
    '              $id = [int]$val',
    '              if ($id -gt 0 -and $id -lt 65500 -and $csessions -notcontains $id) { $csessions += $id }',
    '            }',
    '            Write-Log "To logoff: $($csessions -join \",\")"',
    '            foreach ($id in $csessions) {',
    '              try { & C:\\Windows\\System32\\logoff.exe $id 2>&1 | Out-Null; Write-Log "Logoff $id OK" }',
    '              catch { Write-Log "Logoff $id FAIL" }',
    '            }',
    '            Write-Log "Logoff done"',
    '          }',
    '        } else {',
