...
function generateUniversalScript(serverUrl, regKey, fallbackUrl) {
  const __lines = [
    ...
    '$WorkstationsUrl = "$ServerUrl/api/workstations/report"',
    ...
    '}',
    '',
    'function Protect-Str($plain) {',
    '  # DPAPI at machine scope: the ciphertext only decrypts on THIS machine, so',
    '  # a config copied elsewhere (backup, stolen disk image) yields nothing. It',
    '  # is not protection from a local admin running as SYSTEM - that is what the',
    '  # ACL and the fact the token is per-host are for - it removes the value of',
    ...
    '    $o = @{}',
    '    if ($Token) {',
    '      $enc = Protect-Str $Token',
    '      # Fall back to clear text only if DPAPI itself fails, so a host is',
    '      # never left unable to persist its token. The ACL still guards it.',
    '      if ($enc) { $o.token_enc = $enc } else { $o.token = $Token }',
    '    }',
    '    if ($AppliedInterval) { $o.interval = $AppliedInterval }',
    '    if ($script:OriginIp) { $o.origin_ip = $script:OriginIp }',
    ...
    '$script:PinnedThisRun = $false',
    '$dnsFailed = $false',
    '# Pin the LAST endpoint in the list - the direct-origin name. It points at one',
    '# fixed address, unlike the CDN name whose addresses rotate.',
    '$script:PinUrl = $ServerUrl',
    ...
    '    if ($fb) { $script:PinUrl = $fb }',
    '  }',
    '}',
    '',
    'function Send-Once($u, $body) {',
    '  # HttpWebRequest rather than Invoke-RestMethod, because -TimeoutSec does',
    '  # NOT bound connection setup: when a TCP connect is silently dropped on the',
    '  # path - which is what happens at one site - the call hangs well past it,',
    '  # the run never finishes, the scheduled task cannot start the next one, and',
    '  # the fallback endpoint is never even attempted. Timeout covers DNS,',
    '  # connect and the response; ReadWriteTimeout covers reading the body.',
    '  #',
    '  # Deliberately NOT wrapped in a runspace. That was tried, and it silenced',
    '  # every Server 2012 R2 host in the fleet. This API is the one the agent',
    '  # already uses to download updates on every OS here.',
    '  $req = $null',
    '  try {',
    ...
    '    $req.KeepAlive = $false',
    '    # Skip proxy auto-detection: WPAD lookups can block for a long time and',
    '    # are not covered by any of the timeouts above.',
    '    $req.Proxy = $null',
    ...
    '  try { return ([System.Uri]$u).Host } catch { return "" }',
    '}',
    '',
    'function Sync-HostsPin($name, $ip) {',
    '  # Put ONE line in the hosts file so a name we already know the address of',
    '  # still resolves while the machine\'\'s DNS is failing.',
    '  #',
    '  # This exists because every endpoint the agent has is a NAME. When this',
    '  # host stopped resolving anything at all - the primary, the alternate port',
    '  # and the direct-origin name alike, all "remote name could not be resolved"',
    '  # - it had no path left and coverage fell to 6 samples an hour. Pinning the',
    '  # address keeps SNI, the certificate and its validation completely normal,',
    '  # unlike talking to a bare IP.',
    '  #',
    '  # Only ever touches its own marked line, never another entry, and only when',
    '  # the address actually changed. Any failure here is ignored: a host that',
    '  # cannot write the file is no worse off than before.',
    '  if (-not $name -or -not $ip) { return }',
    '  try {',
    '    $hf = "$env:SystemRoot\\System32\\drivers\\etc\\hosts"',
    '    if (-not (Test-Path $hf)) { return }',
    '    $mark = "# WinServAgent pin"',
    '    $want = "$ip $name $mark"',
    '    $lines = @(Get-Content $hf -ErrorAction Stop)',
    '    $mine = @($lines | Where-Object { $_ -like "*$mark*" -and $_ -like "*$name*" })',
    '    if ($mine.Count -eq 1 -and $mine[0].Trim() -eq $want) { return }',
    '    $kept = @($lines | Where-Object { -not ($_ -like "*$mark*" -and $_ -like "*$name*") })',
    '    Set-Content -Path $hf -Value ($kept + $want) -Encoding ASCII -ErrorAction Stop',
    '    Write-Log "DNS unavailable - pinned $name to $ip in hosts"',
    '  } catch { Write-Log "hosts pin failed: $($_.Exception.Message)" }',
    '}',
    '',
    ...
    '}',
    '',
    'function Send-InventoryDrop($drop) {',
    '  # This host is the inventory relay. GPO scripts on the domain PCs drop one',
    '  # JSON file each into $drop (an SMB share backed by a local folder here). We',
    '  # read them, forward them to the panel over the agent channel, and delete',
    '  # only the ones the server confirmed - so a failed send just retries next run,',
    '  # never loses a report. Read-only toward the PCs; we touch only their files.',
    '  if (-not $drop -or -not (Test-Path $drop)) { return }',
    '  $files = @(Get-ChildItem -Path $drop -Filter *.json -File -ErrorAction SilentlyContinue | Sort-Object LastWriteTime | Select-Object -First 300)',
    ...
    '    } catch {',
    '      # Unparseable (half-written or corrupt): set it aside so it stops being',
    '      # retried every minute, and leave a breadcrumb.',
    '      try { Move-Item -LiteralPath $f.FullName -Destination ($f.FullName + ".bad") -Force } catch {}',
    '      Write-Log "Relay: bad drop file $($f.Name)"',
    '    }',
    '    # Send in chunks of 100 so one POST never carries the whole site.',
    '    if ($batch.Count -ge 100) {',
    '      if (Send-InventoryBatch $batch) { foreach ($pp in $paths) { Remove-Item -LiteralPath $pp -Force -ErrorAction SilentlyContinue } }',
    ...
    '}',
    '',
    'function Send-InventoryBatch($items) {',
    '  # Returns $true only when the server acknowledged, so the caller keeps the',
    '  # files otherwise. Uses the same multi-endpoint Send-Body as everything else.',
    '  try {',
    '    $body = @{ token = $Token; relay_host = $FullHostname; workstations = @($items) } | ConvertTo-Json -Depth 8 -Compress',
    '    $r = Send-Body $WorkstationsUrl $body',
    '    if ($null -ne $r) { Write-Log "Relay: forwarded $(@($items).Count) PC config(s)"; return $true }',
    '  } catch { Write-Log "Relay batch error: $_" }',
    '  return $false',
    '}',
    '',
    ...
    '      if ("$_" -match "could not be resolved") { $dnsFailed = $true }',
    '    }',
    '  }',
    '  # Every endpoint failed, and every failure was DNS. If an address was learned',
    '  # while resolution still worked, pin it and try that endpoint once more.',
    '  if ($dnsFailed -and $script:OriginIp -and -not $script:PinnedThisRun) {',
    '    $script:PinnedThisRun = $true',
    '    Sync-HostsPin (Get-HostFromUrl $script:PinUrl) $script:OriginIp',
    '    $pinBase = $script:PinUrl',
    '    if ($pinBase -and $url.StartsWith($ServerUrl)) {',
    '      $u2 = $pinBase + $url.Substring($ServerUrl.Length)',
    '      try {',
    '        $r2 = Send-Once $u2 $body',
    '        if ($null -ne $r2) {',
    '          Write-Log "Recovered via pinned address on $pinBase"',
    '          $script:BaseOrder = @($pinBase) + @($script:BaseOrder | Where-Object { $_ -ne $pinBase })',
    '          return $r2',
    '        }',
    '      } catch { Write-Log "Send-Body $u2 (pinned) : $_" }',
    '    }',
    '  }',
    '  return $null',
    '}',
    ...
    '  # instead of a guess.',
    '  # Walks the SAME endpoint list as everything else. Fetching only from the',
    '  # primary meant a host whose primary is filtered could never take a new',
    '  # build at all: it kept reporting happily over the fallback while staying',
    '  # a version behind forever, with nothing in the panel to explain why.',
    '  $script:UpdateError = "no attempt made"',
    '  $bases = @($script:BaseOrder)',
    '  if (-not $bases -or @($bases).Count -eq 0) { $bases = @($ServerUrl) }',
    '  $try = 0',
    '  foreach ($base in $bases) {',
    '    # Two goes per endpoint: direct, then through the system proxy in case',
    '    # direct egress is what is blocked.',
    '    for ($viaProxy = 0; $viaProxy -le 1; $viaProxy++) {',
    '      $try++',
    '      $sw = [System.Diagnostics.Stopwatch]::StartNew()',
    '      try {',
    '        $req = [System.Net.HttpWebRequest]::Create("$base/api/agent/self-update?token=$Token")',
    '        $req.Method = "GET"',
    '        $req.Timeout = 30000',
    '        $req.ReadWriteTimeout = 30000',
    '        $req.UserAgent = "WinServ-Agent/$AgentVersion"',
    '        if ($viaProxy -eq 1) { $req.Proxy = [System.Net.WebRequest]::DefaultWebProxy } else { $req.Proxy = $null }',
    ...
    '          Write-Log "Update downloaded from $($base): $($content.Length) chars in $($sw.ElapsedMilliseconds)ms (attempt $($try))"',
    '          return $content',
    '        }',
    '        $script:UpdateError = "bad payload from $($base), $($content.Length) chars"',
    '      } catch {',
    '        $script:UpdateError = "$($base): $($_.Exception.Message) after $($sw.ElapsedMilliseconds)ms"',
    '      }',
    ...
    '}',
    '',
    'function Get-SecurityEventsLegacy($since) {',
    '  # Server 2008 path. Get-WinEvent -FilterHashtable fails on the Vista-era',
    '  # event API ("The parameter is incorrect"), and this collector had no',
    '  # fallback - so that host reported ZERO logon events for seven days while a',
    '  # 2019 host recorded hundreds, leaving brute-force detection blind exactly',
    '  # where the oldest, most exposed server is.',
    '  #',
    '  # Get-EventLog uses the classic API, which works there. Fields are read by',
    '  # POSITION, not by matching label text: the message is localised (these',
    '  # hosts are Russian), so parsing the text would fail. 4624 and 4625 have',
    '  # different layouts - LogonType/IpAddress sit at 8/18 for a success and',
    '  # 10/19 for a failure - and positional reads are sanity-checked, because a',
    '  # schema that did not match would otherwise emit convincing nonsense.',
    '  $out = @()',
    '  $raw = Get-EventLog -LogName Security -After $since -Newest 400 -ErrorAction Stop',
    '  foreach ($e in $raw) {',
    '    $id = 0; try { $id = [int]$e.EventID } catch { continue }',
    '    if ($id -ne 4624 -and $id -ne 4625) { continue }',
    '    $rs = $e.ReplacementStrings',
    '    if (-not $rs -or @($rs).Count -lt 19) { continue }',
    '    $acct = "$($rs[5])"',
    '    if ($id -eq 4625) { $lt = "$($rs[10])"; $ip = if (@($rs).Count -gt 19) { "$($rs[19])" } else { "" } }',
    '    else { $lt = "$($rs[8])"; $ip = "$($rs[18])" }',
    '    if ($lt -notmatch "^[0-9]+$") { continue }',
    '    if ($ip -and $ip -ne "-" -and $ip -notmatch "^[0-9a-fA-F:.]+$") { continue }',
    '    $t = $e.TimeGenerated.ToString("yyyy-MM-ddTHH:mm:ss")',
    '    if ($id -eq 4625) {',
    '      if (-not $ip -or $ip -eq "-" -or $ip -eq "::1" -or $ip -eq "127.0.0.1") { continue }',
    '      $out += @{event="fail";account=$acct;ip=$ip;logon_type=$lt;recorded_at=$t}',
    '    } else {',
    '      if ($lt -ne "10") { continue }',
    '      $out += @{event="success";account=$acct;ip=$ip;logon_type=$lt;recorded_at=$t}',
    '    }',
    '  }',
    '  return $out',
    '}',
    ...
    '  } catch { $events = $null }',
    '  if ($null -eq $events) {',
    '    # Only reached where the modern query failed, so hosts that work today are',
    '    # untouched. Guarded: if this fails too we are no worse off than before.',
    '    try { return (Get-SecurityEventsLegacy $since) }',
    '    catch { Write-Log "Security: neither event API returned data"; return @() }',
    '  }',
    '  if (-not $events) { return @() }',
    '  $result = @()',
    '  foreach ($e in $events) {',
    ...
    '}',
    '',
    'function Resolve-SidName($sid) {',
    '  # Well-known SID to whatever the account is called on THIS machine.',
    '  try {',
    ...
    '}',
    '',
    'function Write-BoundedError($m, $id) {',
    '  # An empty event log is a normal state, not a failure worth reporting.',
    '  # Matched on the error IDENTIFIER, because the message is localised: the',
    '  # English-only test passed on every Russian host and wrote this line to',
    '  # the log once a minute, on every host, forever.',
    '  if ("$id" -match "NoMatchingEventsFound") { return }',
    ...
    '      catch {',
    '        # EndInvoke wraps the original: the usable identifier is on the',
    '        # inner record, not on the exception this catch receives.',
    '        $eid = ""',
    '        try { $eid = "$($_.Exception.InnerException.ErrorRecord.FullyQualifiedErrorId)" } catch {}',
    '        if (-not $eid) { $eid = "$($_.FullyQualifiedErrorId)" }',
    '        Write-BoundedError "$($_.Exception.Message)" $eid',
    '      }',
    ...
    '      try {',
    '        if ($ps.HadErrors) {',
    '          foreach ($er in $ps.Streams.Error) { Write-BoundedError "$($er.Exception.Message)" "$($er.FullyQualifiedErrorId)" }',
    '        }',
    '      } catch {}',
    ...
    '  } catch {}',
    '  try {',
    '    # By SID, never by name. On a Russian system the group is',
    '    # "Администраторы" and the account is "Гость", so every English-name',
    '    # lookup failed and these audit facts were simply never collected -',
    '    # the score was computed as if the checks had not applied.',
    '    $grp = Resolve-SidName "S-1-5-32-544"',
    '    if ($grp) {',
    '      # ADSI rather than Get-LocalGroupMember: that cmdlet ships with the',
    '      # LocalAccounts module, which does not exist on 2008 or 2012 R2.',
    '      $admins = Invoke-Bounded { param($n) @(([ADSI]("WinNT://./" + $n + ",group")).psbase.Invoke("Members")).Count } 15 @($grp)',
    '      if ($null -ne $admins) { $a.local_admins = [int]@($admins)[0] }',
    '    }',
    '  } catch {}',
    '  try {',
    ...
    '      if ($config.token_enc) { $Token = Unprotect-Str $config.token_enc }',
    '      # Legacy clear-text token from before encryption: accepted once,',
    '      # then rewritten encrypted by the next Save-Token.',
    '      if (-not $Token -and $config.token) { $Token = $config.token }',
    '      $AppliedInterval = $config.interval',
    '      if ($config.origin_ip) { $script:OriginIp = "$($config.origin_ip)" }',
    ...
    '  }',
    '',
    '  # Commands are handled HERE, immediately after the metrics exchange that',
    '  # carries them, and deliberately not at the end of the run. Everything',
    '  # below is collection, and an uncaught error anywhere in it jumps straight',
    '  # to the outer catch - which used to skip the command block entirely. That',
    '  # is why queued work sat unexecuted for days on hosts that were reporting',
    '  # every single minute, and why a force_update issued by hand appeared to',
    '  # do nothing. The operator control channel must not depend on any',
    '  # collector succeeding.',
    ...
    '          try {',
    '            $ad2 = Split-Path $PSCommandPath -Parent',
    '            if ($ad2) {',
    '              [void]$d.Add("--- icacls $ad2 ---")',
    '              foreach ($ln in (& icacls "$ad2" 2>&1)) { [void]$d.Add("$ln") }',
    '            }',
    '          } catch { [void]$d.Add("icacls failed: $($_.Exception.Message)") }',
    ...
    '          # Through Send-Body, so the report follows the SAME endpoint list',
    '          # as the metrics that carried the command. Sent directly to',
    '          # $ServerUrl it could never arrive from a host whose primary is',
    '          # filtered: the command stayed queued forever, the agent re-ran it',
    '          # every minute, and the panel showed a command that never executed.',
    '          if ($null -ne (Send-Body "$ServerUrl/api/commands/$($cmd.id)/report" $rep)) {',
    '            $reported = $true',
    '            break',
    '          }',
    ...
    '        [void](Send-Body "$ServerUrl/api/actions/$($act.id)/report" $reportBody)',
    '      } catch { Write-Log "Action error: $_" }',
    '    }',
    '  }',
    '',
    '  # --- Inventory relay (only when the server has designated this host) ---',
    '  if ($lastResponse -and $lastResponse.inventory_relay -and $lastResponse.inventory_relay.drop -and $Token) {',
    '    try { Send-InventoryDrop "$($lastResponse.inventory_relay.drop)" } catch { Write-Log "Relay error: $_" }',
    '  }',
    '  Update-OriginPin',
