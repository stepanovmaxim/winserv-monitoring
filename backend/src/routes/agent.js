...
function generateUniversalScript(serverUrl, regKey) {
  return [
    ...
    '            Write-Log "Sessions raw: $([string]::Join(\" | \", ($raw -split \"`n\")))"',
    '            $csessions = $raw -replace "^[> ]", "" | ForEach-Object { if ($_ -match "^\s*\S+\s+(\d+)\s") { [int]$matches[1] } }',
    '            Write-Log "Session IDs found: $([string]::Join(\", \", $csessions))"',
    '            foreach ($id in $csessions) {',
    '              if ($id -gt 0) {',
    '                try { & C:\\Windows\\System32\\logoff.exe $id 2>&1 | Out-Null; Write-Log "Logged off session $id" }',
    '                catch { Write-Log "Logoff failed for session $id : $_" }',
    '              }',
    '            }',
