...
function generateUniversalScript(serverUrl, regKey) {
  return [
    ...
    '            $csessions = @()',
    '            try {',
    '              $all = Get-CimInstance -Namespace root/cimv2/TerminalServices -ClassName Win32_TSSession -ErrorAction Stop',
    '              foreach ($s in $all) {',
    '                if ($s.SessionId -gt 0 -and $s.SessionId -ne $PID -and $s.ConnectionState -ne 0) {',
    '                  $csessions += $s.SessionId',
    '                }',
    '              }',
    '            } catch {}',
    '            if ($csessions.Count -eq 0) {',
    '              $raw = @(quser 2>$null)',
    '              foreach ($l in $raw) {',
    '                $arr = "$l" -split "\s+"',
    '                foreach ($p in $arr) {',
    '                  $isNum = [int]::TryParse($p, [ref]$null)',
    '                  if ($isNum) { $i = [int]$p; if ($i -gt 0 -and $i -lt 65500) { $csessions += $i } }',
    '                }',
    '              }',
    '            }',
    '            $csessions = $csessions | Select-Object -Unique',
