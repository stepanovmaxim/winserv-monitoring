...
export default function Users() {
  ...
  function accessCell(u) {
    ...
    if (!ids.length) return <span style={{ fontSize: 12, color: 'var(--warning)' }}>{t('usr.allCustomers')}</span>;
    return (
  ...
  return (
    <div>
      <div className="page-header"><h1>{t('usr.title')}</h1></div>

      <div className="card" style={{ marginBottom: 16 }}>
        <p style={{ color: 'var(--text-muted)' }}>{t('usr.desc')}</p>
      </div>

      <div className="card">
        {users.length === 0 ? (
          <div className="empty"><div className="empty-icon">👥</div><p>{t('usr.empty')}</p></div>
        ) : (
          <table>
            <thead><tr><th>{t('usr.user')}</th><th>{t('usr.email')}</th><th>{t('usr.role')}</th><th>{t('usr.access')}</th><th>{t('usr.registered')}</th><th></th></tr></thead>
            <tbody>
              {users.map(u => (
                <tr key={u.id}>
                  <td>
                  ...
                  <td style={{ whiteSpace: 'nowrap' }}>
                    ...
                    >
                      <option value="admin">{t('usr.r.admin')}</option>
                      <option value="viewer">{t('usr.r.viewer')}</option>
                      <option value="pending">{t('usr.r.pending')}</option>
                    </select>
                    {u.role !== 'pending' && (
                      <button className="secondary" style={{ padding: '4px 10px', fontSize: 12, marginLeft: 6 }} onClick={() => openAccess(u)}>{t('usr.accessBtn')}</button>
                    )}
                  </td>
                </tr>
              ))}
      ...
      {editing && (
        <div className="modal-overlay" onClick={() => setEditing(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h2>{t('usr.accessTitle', { who: editing.name || editing.email })}</h2>
            <p style={{ color: 'var(--text-muted)', fontSize: 13, marginBottom: 16 }}>{t('usr.accessDesc')}</p>

            {customers.length === 0 ? (
              <div className="empty"><p>{t('usr.noCustomers')}</p></div>
            ) : (
              <div style={{ maxHeight: 320, overflowY: 'auto', border: '1px solid var(--border)', borderRadius: 8, padding: 10 }}>
                {customers.map(c => (
                  <label key={c.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 2px', cursor: 'pointer' }}>
                    ...
                    <span>{c.name}</span>
                    <span style={{ marginLeft: 'auto', fontSize: 12, color: 'var(--text-muted)' }}>{t('usr.serversN', { n: c.server_count })}</span>
                  </label>
                ))}
              </div>
            )}
            ...
            <p style={{ fontSize: 12, color: picked.length ? 'var(--text-muted)' : 'var(--warning)', marginTop: 10 }}>
              {picked.length ? t('usr.restricted', { n: picked.length }) : t('usr.noRestriction')}
            </p>
            {error && <p style={{ color: 'var(--danger)', fontSize: 13 }}>{error}</p>}

            <div className="form-actions">
              <button type="button" onClick={saveAccess} disabled={saving}>{t('common.save')}</button>
