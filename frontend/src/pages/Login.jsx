import { useState, useEffect } from 'react';
import { useNavigate, useSearchParams } from 'react-router-dom';
import { useAuth } from '../context/AuthContext';
import { api } from '../api';

export default function Login() {
  const { user, login } = useAuth();
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const [email, setEmail] = useState('');
  const [error, setError] = useState('');

  useEffect(() => {
    if (user) navigate('/');

    const token = searchParams.get('token');
    if (token) {
      login(token).then(() => navigate('/')).catch(err => setError(err.message));
    }
  }, [user]);

  async function handleDevLogin(e) {
    e.preventDefault();
    setError('');
    try {
      const { token } = await api.devLogin(email);
      await login(token);
      navigate('/');
    } catch (err) {
      setError(err.message);
    }
  }

  function handleGoogleLogin() {
    window.location.href = '/api/auth/google';
  }

  return (
    <div className="login-page">
      <div className="login-card">
        <h1>WinServ Monitoring</h1>
        <p>Sign in to monitor your servers</p>

        {process.env.NODE_ENV === 'development' || !import.meta.env.VITE_GOOGLE_CLIENT_ID ? (
          <form onSubmit={handleDevLogin}>
            <div className="form-group">
              <label>Email (dev mode)</label>
              <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="Stepanov.maxim@gmail.com" />
            </div>
            <button type="submit" style={{ width: '100%' }}>Dev Login</button>
            {error && <p style={{ color: 'var(--danger)', marginTop: 12 }}>{error}</p>}
          </form>
        ) : null}

        {import.meta.env.VITE_GOOGLE_CLIENT_ID ? (
          <>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, margin: '20px 0' }}>
              <div style={{ flex: 1, height: 1, background: 'var(--border)' }} />
              <span style={{ color: 'var(--text-muted)', fontSize: 13 }}>or</span>
              <div style={{ flex: 1, height: 1, background: 'var(--border)' }} />
            </div>
            <button className="google-btn" onClick={handleGoogleLogin}>
              <svg width="20" height="20" viewBox="0 0 24 24"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"/><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/></svg>
              Sign in with Google
            </button>
          </>
        ) : null}

        {error && <p style={{ color: 'var(--danger)', marginTop: 12 }}>{error}</p>}
      </div>
    </div>
  );
}
