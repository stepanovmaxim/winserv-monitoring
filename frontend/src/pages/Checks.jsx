    ...
    <div>
      ...
      <div className="card">
        {checks.length === 0 ? (
          ...
          <table>
            <thead><tr><th>Status</th><th>Name</th><th>Type</th><th>Target</th><th>Latency</th><th>Customer</th><th>Checked</th>{user?.role === 'admin' && <th></th>}</tr></thead>
            <tbody>
              {checks.map(c => (
                <tr key={c.id}>
                  ...
                  <td>{c.kind}</td>
                  <td style={{ fontFamily: 'monospace', fontSize: 12 }}>{c.host}{c.port ? `:${c.port}` : ''}</td>
        ...
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={save}>
              ...
              <div className="form-group"><label>Type</label><select value={form.kind} onChange={e => setForm({ ...form, kind: e.target.value })}><option value="ping">Ping (ICMP)</option><option value="tcp">TCP port</option></select></div>
              <div className="form-group"><label>Host / IP *</label><input value={form.host} onChange={e => setForm({ ...form, host: e.target.value })} required placeholder="1.2.3.4 or host.example.com" /></div>
